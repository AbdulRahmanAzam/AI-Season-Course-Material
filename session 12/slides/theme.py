...
RED = RGBColor(0xB3, 0x26, 0x1E)      # only for "bad / no / broken" marks
...
FOOTER = "AI Season   ·   Agent-to-Agent (A2A) Communication   ·   Abdul Rahman Azam"
...
def _runs(par, text, size, color=INK, bold=False, font=BODY):
    """Fill a paragraph, honouring the markup. A "\\n" becomes a soft line break."""
    for i, line in enumerate(str(text).split("\n")):
        if i:
            par.add_line_break()
        _runs_line(par, line, size, color, bold, font=font)


def _runs_line(par, text, size, color=INK, bold=False, italic=False, font=BODY):
    """One line: **bold**, *italic*, `code` and ~~orange~~.

    The wrappers nest, so `code` inside **bold** still renders as monospace
    instead of printing its own backticks.
    """
    for piece in _TOK.split(text):
        if not piece:
            continue
        if piece.startswith("**") and piece.endswith("**"):
            _runs_line(par, piece[2:-2], size, color, True, italic, font)
            continue
        if piece.startswith("~~") and piece.endswith("~~"):
            _runs_line(par, piece[2:-2], size, ORANGE, True, italic, font)
            continue
        if piece.startswith("*") and piece.endswith("*"):
            _runs_line(par, piece[1:-1], size, color, bold, True, font)
            continue
...
# Consolas puts its line box at about 1.17 em; code() sets line_spacing 1.22.
_CODE_LINE = 1.17 * 1.22 / 72.0          # inches of height per point of size


def code_h(n_lines, size=11.5, title=True):
    """The height a code() panel needs. Guessing this by hand is how blocks end
    up sitting on top of each other, so always ask for it."""
    return 0.13 + (0.28 if title else 0.0) + n_lines * size * _CODE_LINE + 0.14


def code(s, x, y, w, h, lines, size=11.5, title=None, fill=CODEBG):
    """A monospace panel with an orange left edge. h=None sizes it to fit."""
    if h is None:
        h = code_h(len(lines), size, title)
...
def chip_row(s, x, y, w, h, items, gap=0.34, arrows=True, size=12,
             fill=WHITE, edge=ORANGE, arrow_h=0.20):
    """A left-to-right row of evenly spaced chips, optionally arrow-linked.

    items: "text", or ("text", "sub-caption"), or ("text", "sub", fill).
    """
    n = len(items)
    cw = (w - gap * (n - 1)) / n
    out = []
    for i, it in enumerate(items):
        if isinstance(it, str):
            text, sub, f = it, None, fill
        elif len(it) == 2:
            (text, sub), f = it, fill
        else:
            text, sub, f = it
        cx = x + i * (cw + gap)
        out.append(chip(s, cx, y, cw, h, text, sub=sub, size=size, fill=f, edge=edge))
        if arrows and i < n - 1:
            arrow(s, cx + cw + 0.04, y + h / 2 - arrow_h / 2, gap - 0.08, arrow_h)
    return out
...
def arrow(s, x, y, w, h, direction="right", color=ORANGE_HI):
    """A solid arrow. A2A diagrams need replies, so "left" and "up" matter here."""
    shp = s.shapes.add_shape(_ARROWS[direction],
...
def vrule(s, x, y, h, color=LINE, thick=0.012):
    """A vertical hairline — sequence-diagram lifelines and tree stems."""
    shp = s.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(x), Inches(y),
                             Inches(thick), Inches(h))
    shp.fill.solid()
    shp.fill.fore_color.rgb = color
    _noline(shp)
    _noshadow(shp)
